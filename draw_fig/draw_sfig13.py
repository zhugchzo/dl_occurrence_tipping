...
legend_resids = mlines.Line2D([], [], color='crimson', label='Residual', linestyle='-', alpha=0.5)
...
may_fold_resids_list = list()
may_fold_b_list = list()
food_hopf_resids_list = list()
food_hopf_b_list = list()
cr_branch_resids_list = list()
cr_branch_b_list = list()
global_fold_resids_list = list()
global_fold_b_list = list()
MPT_hopf_resids_list = list()
MPT_hopf_b_list = list()
amazon_branch_resids_list = list()
amazon_branch_b_list = list()
...
for c_rate in [1e-5,2e-5,3e-5,4e-5,5e-5]:
    ...
    for n in range(1,numtest+1):
        ...
        may_fold_resids_list.append(np.mean(saliency_resids))
        may_fold_b_list.append(np.mean(saliency_b))
...
for c_rate in [1e-5,2e-5,3e-5,4e-5,5e-5]:
    ...
    for n in range(1,numtest+1):
        ...
        food_hopf_resids_list.append(np.mean(saliency_resids))
        food_hopf_b_list.append(np.mean(saliency_b))
...
for c_rate in [1e-4,2e-4,3e-4,4e-4,5e-4]:
    ...
    for n in range(1,numtest+1):
        ...
        cr_branch_resids_list.append(np.mean(saliency_resids))
        cr_branch_b_list.append(np.mean(saliency_b))
...
for c_rate in [-5e-7,-6e-7,-7e-7,-8e-7,-9e-7]:
    ...
    for n in range(1,numtest+1):
        ...
        global_fold_resids_list.append(np.mean(saliency_resids))
        global_fold_b_list.append(np.mean(saliency_b))
...
for c_rate in [1e-5,2e-5,3e-5,4e-5,5e-5]:
    ...
    for n in range(1,numtest+1):
        ...
        MPT_hopf_resids_list.append(np.mean(saliency_resids))
        MPT_hopf_b_list.append(np.mean(saliency_b))
...
for c_rate in [-1e-5,-2e-5,-3e-5,-4e-5,-5e-5]:
    ...
    for n in range(1,numtest+1):
        ...
        amazon_branch_resids_list.append(np.mean(saliency_resids))
        amazon_branch_b_list.append(np.mean(saliency_b))
...
plt.savefig('../figures/SFIG13.pdf',format='pdf',dpi=600)



print('may fold: {}, {}'.format(np.mean(may_fold_resids_list),np.mean(may_fold_b_list)))
print('food hopf: {}, {}'.format(np.mean(food_hopf_resids_list),np.mean(food_hopf_b_list)))
print('cr branch: {}, {}'.format(np.mean(cr_branch_resids_list),np.mean(cr_branch_b_list)))
print('global fold: {}, {}'.format(np.mean(global_fold_resids_list),np.mean(global_fold_b_list)))
print('MPT hopf: {}, {}'.format(np.mean(MPT_hopf_resids_list),np.mean(MPT_hopf_b_list)))
print('amazon branch: {}, {}'.format(np.mean(amazon_branch_resids_list),np.mean(amazon_branch_b_list)))