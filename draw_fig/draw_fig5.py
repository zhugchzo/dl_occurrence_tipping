...
for p in range(3):
    ...
    subplt.set_xlabel('Light irradiance ($\\mu$mol photons m$^{-2}$ s$^{-1}$)',font_axis)
    subplt.set_ylabel('Light attenuation coefficient (m$^{-1}$)',font_axis)
...
for p in range(3):
    ...
    subplt.tick_params(axis='both', labelsize=10)
    subplt.set_xlabel('Voltage (V)',font_axis)
    subplt.set_ylabel('Acoustic pressure (Pa)',font_axis)

plt.subplots_adjust(top=0.9, bottom=0.1, left=0.07, right=0.99, hspace=0.48, wspace=0.25)
