...
# combined model
preds_combined_1 = df_combined['preds_20'].values
preds_combined_2 = df_combined['preds_21'].values
preds_combined_3 = df_combined['preds_22'].values
...
for p in range(len(thermoacoustic_par_range_list)):
    ...
    subplt.axvline(preds_combined,color='darkorange',linestyle=':',label='Combined Model',alpha=0.9)
...
plt.subplots_adjust(top=0.9, bottom=0.055, left=0.07, right=0.99, hspace=0.4, wspace=0.5)

handles, labels = axs[0, 0].get_legend_handles_labels()
fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5,1), ncol=3, frameon=False, fontsize=12)

plt.savefig('../figures/SFIG18.png',dpi=600)
