...
def convert_bifdata(filepath):
    ...
    f = open(filepath, 'r')
    raw_lines = f.readlines()#f按行返回到list
    f.close()
    ...
    types = [int(raw1[i][13]) for i in range(len(raw1))]
    
    # Get bif param values
    bifParam = [float(placeE(raw1[i][20:38])) for i in range(len(raw1))]
    
    # Get x values           
    xVals = [float(placeE(raw1[i][58:76])) for i in range(len(raw1))]
