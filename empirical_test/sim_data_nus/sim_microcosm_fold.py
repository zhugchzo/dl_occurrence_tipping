...
# dataset[:4935]  0-17.5
# dataset[4947:7304]  18.3-26.695 (the tipping point)

sample_start_1 = [0,1000,2000,3000,4000]
sample_start_2 = [5000,6000]

for ss in sample_start_1:

    # Create directories for output
    if not os.path.exists('../data_nus/microcosm_fold/{}'.format(ss)):
        os.makedirs('../data_nus/microcosm_fold/{}'.format(ss))

    dataset_1 = np.array(dataset[ss:4935])

    tmax = 400

    d = np.arange(0,len(dataset_1))
    np.random.shuffle(d)
    d = list(np.sort(d[0:tmax]))
    dataset_dl = dataset_1[d]
    ...
    filepath_resids='../data_nus/microcosm_fold/{}/microcosm_fold_resids.csv'.format(ss)
    ...
    filepath_interpolate='../data_nus/microcosm_fold/{}/microcosm_fold_interpolate.csv'.format(ss)
    ...
    filepath_ac = '../data_nus/microcosm_fold/{}/microcosm_fold_ac.csv'.format(ss)
    ...
    filepath_dev = '../data_nus/microcosm_fold/{}/microcosm_fold_dev.csv'.format(ss)
    df_dev.to_csv(filepath_dev)

for ss in sample_start_2:

    # Create directories for output
    if not os.path.exists('../data_nus/microcosm_fold/{}'.format(ss)):
        os.makedirs('../data_nus/microcosm_fold/{}'.format(ss))

    dataset_2 = np.array(dataset[ss:7304])
    ...
    d = np.arange(0,len(dataset_2))
    np.random.shuffle(d)
    d = list(np.sort(d[0:series_len]))
    dataset_dl = dataset_2[d]
