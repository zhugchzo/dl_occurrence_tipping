...
if not os.path.exists('../data_nus/hypoxia_64PE_U_branch'):
    os.makedirs('../data_nus/hypoxia_64PE_U_branch')

par_range_sum = ['300-268','298-266','296-264','294-262']

for c in range(numSims):

    interpolated_df = pd.read_csv('../data_nus/hypoxia_64PE_U_branch/hypoxia_64PE_U_branch_400_interpolate_{}.csv'.format(par_range_sum[c]))
    ...
    filepath_dev = '../data_nus/hypoxia_64PE_U_branch/hypoxia_64PE_U_branch_400_dev_{}.csv'.format(par_range_sum[c])
    df_dev.to_csv(filepath_dev,index=False)

    # Export lag-1 autocorrelation red as individual files for BB method
    filepath_ac = '../data_nus/hypoxia_64PE_U_branch/hypoxia_64PE_U_branch_400_ac_{}.csv'.format(par_range_sum[c])
