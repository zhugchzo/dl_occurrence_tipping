...
single_train_numbers = 25000 # volume of training data for each changing direction of parameter (increase / decrease) and noise type (white / red)
...
train_numbers = int(single_train_numbers)*4 # volume of training data 25000*4
...
for i in range(single_train_numbers):
    ...
    targets_branch_f_white_value = (targets_branch_f_value - normalization[4*i][0])/normalization[4*i][1]
    targets_branch_f_red_value = (targets_branch_f_value - normalization[4*i+1][0])/normalization[4*i+1][1]
    targets_branch_r_white_value = (targets_branch_r_value - normalization[4*i+2][0])/normalization[4*i+2][1]
    targets_branch_r_red_value = (targets_branch_r_value - normalization[4*i+3][0])/normalization[4*i+3][1]
...
groups_name='/increased_bifurcation/output/ts_500/branch/combined/groups.csv'
groups = pandas.read_csv(groups_name,header = 0)
groups = groups.values[:single_train_numbers,1]
groups = np.concatenate([groups,groups,groups,groups])
...
model_name = "best_model_branch_"+str(kk_num)+".keras"
