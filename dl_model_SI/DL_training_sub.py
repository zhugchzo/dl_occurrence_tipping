...
zf_sub_pitchfork = zipfile.ZipFile('/gs/home/by2109123/gen_pitchfork/output/sub_pitchfork/combined/output_sims.zip')
...
for i in range (1,set_size):
    ...
    df_sub_pitchfork = pandas.read_csv(zf_sub_pitchfork.open('output_sims/sub_tseries'+str(i)+'.csv'))

    keep_col = ['x','b']

    new_sub_pitchfork = df_sub_pitchfork[keep_col]

    values_sub_pitchfork = new_sub_pitchfork.values

    values_sub_pitchfork_over = df_sub_pitchfork['b'].iloc[-1]

    sequences.append(values_sub_pitchfork)

    normalization.append([0,values_sub_pitchfork_over])
...
label_sub_pitchfork='/gs/home/by2109123/gen_pitchfork/output/sub_pitchfork/combined/values.csv'
targets_sub_pitchfork = pandas.read_csv(label_sub_pitchfork,header = 0)

targets_value = []
for i in range(train_numbers):

    targets_sub_pitchfork_value = targets_sub_pitchfork.iloc[i,2]

    targets_sub_pitchfork_value = (targets_sub_pitchfork_value - normalization[i][0])/normalization[i][1]

    targets_value.append(targets_sub_pitchfork_value)

targets_value = np.array(targets_value)

# train/validation/test split denotations
groups_name='/gs/home/by2109123/gen_pitchfork/output/sub_pitchfork/combined/groups.csv'
