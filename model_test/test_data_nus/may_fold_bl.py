...
for b in np.linspace(0,0.2,11):
    ...
    for c_rate in [1e-5,2e-5,3e-5,4e-5,5e-5]:
        ...
        for n in range (1,numtest+1):
            ...
            df_ac = pandas.read_csv('../data_nus/may_fold_white/{}/{}/ac/may_fold_ac_'.format(bl,c_rate)+str(n)+'.csv')
            df_dev = pandas.read_csv('../data_nus/may_fold_white/{}/{}/dev/may_fold_dev_'.format(bl,c_rate)+str(n)+'.csv')
...
csv_bl_out.to_csv('../../results/may_fold/may_fold_bl_nus.csv',header = True)
...
csv_bh_out.to_csv('../../results/may_fold/may_fold_bh_nus.csv',header = True)