...
for c_rate in [1e-5,2e-5,3e-5,4e-5,5e-5]:
    ...
    for k in np.linspace(0.2,0.4,11):
        ...
        for i in range (1,numtest+1):
            df_resids = pandas.read_csv('../data_nus/food_hopf_white/{}/{}/resids/food_hopf_resids_'.format(kl,c_rate)+str(i)+'.csv')
...
csv_out.to_csv('../../results/food_hopf/food_hopf_crate_nus.csv',header = True)