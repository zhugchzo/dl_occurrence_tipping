...
for u in np.linspace(0,0.3,11):
    ...
    for c_rate in [1e-5,2e-5,3e-5,4e-5,5e-5]:
        ...
        for n in range (1,numtest+1):
            ...
            df_ac = pandas.read_csv('../data_nus/MPT_hopf_red/{}/{}/ac/MPT_hopf_ac_'.format(ul,c_rate)+str(n)+'.csv')
            df_dev = pandas.read_csv('../data_nus/MPT_hopf_red/{}/{}/dev/MPT_hopf_dev_'.format(ul,c_rate)+str(n)+'.csv')
...
csv_ul_out.to_csv('../../MPT_hopf/MPT_hopf_ul_nus.csv',header = True)
...
csv_uh_out.to_csv('../../results/MPT_hopf/MPT_hopf_uh_nus.csv',header = True)