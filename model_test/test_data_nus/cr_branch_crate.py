...
for c_rate in [1e-4,2e-4,3e-4,4e-4,5e-4]:
    ...
    for a in np.linspace(0,5,11):
        ...
        for i in range (1,numtest+1):
            df_resids = pandas.read_csv('../data_nus/cr_branch_white/{}/{}/resids/cr_branch_resids_'.format(al,c_rate)+str(i)+'.csv')
...
csv_out.to_csv('../../results/cr_branch/cr_branch_crate_nus.csv',header = True)