...
for i in np.linspace(0.2,0.4,11):
    ...
    for i in range (1,numtest+1):
        df_resids = pandas.read_csv('../data_nus/food_hopf_white/{}/resids/food_hopf_500_resids_'.format(kl)+str(i)+'.csv')
        df_ac = pandas.read_csv('../data_nus/food_hopf_white/{}/ac/food_hopf_500_ac_'.format(kl)+str(i)+'.csv')
        df_dev = pandas.read_csv('../data_nus/food_hopf_white/{}/dev/food_hopf_500_dev_'.format(kl)+str(i)+'.csv')
    ...
    result = pandas.read_csv('../data_nus/food_hopf_white/{}/food_hopf_500_result.csv'.format(kl),header=0)
    ...
    for j in range(numtest):
        ...
        #fit curve for AC
        param_ac = np.polyfit(sequences_ac[j],sequences_kac[j],2)
        ...
        #fit curve for DEV
        param_dev = np.polyfit(sequences_dev[j],sequences_kdev[j],2)
...
csv_out.to_csv('../../results/food_hopf_nus.csv',header = True)