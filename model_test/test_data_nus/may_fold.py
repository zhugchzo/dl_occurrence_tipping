...
for i in np.linspace(0,0.2,11):
    ...
    for i in range (1,numtest+1):
        df_resids = pandas.read_csv('../data_nus/may_fold_white/{}/resids/may_fold_500_resids_'.format(bl)+str(i)+'.csv')
        df_ac = pandas.read_csv('../data_nus/may_fold_white/{}/ac/may_fold_500_ac_'.format(bl)+str(i)+'.csv')
        df_dev = pandas.read_csv('../data_nus/may_fold_white/{}/dev/may_fold_500_dev_'.format(bl)+str(i)+'.csv')
    ...
    result = pandas.read_csv('../data_nus/may_fold_white/{}/may_fold_500_result.csv'.format(bl),header=0)
    ...
    for j in range(numtest):
        ...
        #fit curve for AC
        param_ac = np.polyfit(sequences_ac[j],sequences_bac[j],2)
        ...
        #fit curve for DEV
        param_dev = np.polyfit(sequences_dev[j],sequences_bdev[j],2)
...
csv_out.to_csv('../../results/may_fold_nus.csv',header = True)