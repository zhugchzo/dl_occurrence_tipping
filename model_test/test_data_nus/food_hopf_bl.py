...
for k in np.linspace(0.2,0.4,11):
    ...
    for c_rate in [1e-5,2e-5,3e-5,4e-5,5e-5]:
        ...
        for n in range (1,numtest+1):
            ...
            df_ac = pandas.read_csv('../data_nus/food_hopf_white/{}/{}/ac/food_hopf_ac_'.format(kl,c_rate)+str(n)+'.csv')
            df_dev = pandas.read_csv('../data_nus/food_hopf_white/{}/{}/dev/food_hopf_dev_'.format(kl,c_rate)+str(n)+'.csv')
...
csv_kl_out.to_csv('../../results/food_hopf/food_hopf_kl_nus.csv',header = True)
...
csv_kh_out.to_csv('../../results/food_hopf/food_hopf_kh_nus.csv',header = True)