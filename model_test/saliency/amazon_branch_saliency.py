...
if not os.path.exists('../../results/saliency/amazon_branch'):
    os.makedirs('../../results/saliency/amazon_branch')

for c_rate in [-1e-5,-2e-5,-3e-5,-4e-5,-5e-5]:

    for n in range(1,numtest+1):

        df_resids = pandas.read_csv('../data_nus/amazon_branch_red/{}/{}/resids/amazon_branch_resids_'.format(0.9,c_rate)+str(n)+'.csv')
        ...
        csv_saliency_out.to_csv('../../results/saliency/amazon_branch/saliency_feature_{}_{}.csv'.format(c_rate,n),header = True)