...
if not os.path.exists('../../results/saliency/MPT_hopf'):
    os.makedirs('../../results/saliency/MPT_hopf')

for c_rate in [1e-5,2e-5,3e-5,4e-5,5e-5]:

    for n in range(1,numtest+1):

        df_resids = pandas.read_csv('../data_nus/MPT_hopf_red/{}/{}/resids/MPT_hopf_resids_'.format(0.0,c_rate)+str(n)+'.csv')
        ...
        csv_saliency_out.to_csv('../../results/saliency/MPT_hopf/saliency_feature_{}_{}.csv'.format(c_rate,n),header = True)