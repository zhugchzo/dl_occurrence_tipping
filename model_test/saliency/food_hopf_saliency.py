...
if not os.path.exists('../../results/saliency/food_hopf'):
    os.makedirs('../../results/saliency/food_hopf')

for c_rate in [1e-5,2e-5,3e-5,4e-5,5e-5]:

    for n in range(1,numtest+1):

        df_resids = pandas.read_csv('../data_nus/food_hopf_white/{}/{}/resids/food_hopf_resids_'.format(0.2,c_rate)+str(n)+'.csv')
        ...
        csv_saliency_out.to_csv('../../results/saliency/food_hopf/saliency_feature_{}_{}.csv'.format(c_rate,n),header = True)