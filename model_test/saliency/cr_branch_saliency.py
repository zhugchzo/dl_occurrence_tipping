...
if not os.path.exists('../../results/saliency/cr_branch'):
    os.makedirs('../../results/saliency/cr_branch')

for c_rate in [1e-4,2e-4,3e-4,4e-4,5e-4]:

    for n in range(1,numtest+1):

        df_resids = pandas.read_csv('../data_nus/cr_branch_white/{}/{}/resids/cr_branch_resids_'.format(0.0,c_rate)+str(n)+'.csv')
        ...
        csv_saliency_out.to_csv('../../results/saliency/cr_branch/saliency_feature_{}_{}.csv'.format(c_rate,n),header = True)