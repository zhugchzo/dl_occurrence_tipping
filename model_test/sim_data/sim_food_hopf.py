...
for i in np.linspace(0.2,0.4,11):
    ...
    while j < numSims:
        ...
        n = np.random.us(100,500) # length of the randomly selected sequence to the bifurcation
    ...
    if not os.path.exists('../data_nus/food_hopf_white/{}/temp'.format(kl)):
        os.makedirs('../data_nus/food_hopf_white/{}/temp'.format(kl))

    if not os.path.exists('../data_us/food_hopf_white/{}/temp'.format(kl)):
        os.makedirs('../data_us/food_hopf_white/{}/temp'.format(kl))

    # Filter time-series
    df_traj1.to_csv('../data_nus/food_hopf_white/{}/temp/df_traj.csv'.format(kl))
    df_traj2.to_csv('../data_us/food_hopf_white/{}/temp/df_traj.csv'.format(kl))

    if not os.path.exists('../data_nus/food_hopf_white/{}/dev'.format(kl)):
        os.makedirs('../data_nus/food_hopf_white/{}/dev'.format(kl))

    if not os.path.exists('../data_us/food_hopf_white/{}/dev'.format(kl)):
        os.makedirs('../data_us/food_hopf_white/{}/dev'.format(kl))
    ...
    for i in range(numSims):
        ...
        df_traj_k = df_traj1[df_traj1['tsid'] == i+1]['k']
        ...
        filepath_dev = '../data_nus/food_hopf_white/{}/dev/food_hopf_500_dev_{}.csv'.format(kl,i+1)
    ...
    for i in range(numSims):
        ...
        df_traj_k = df_traj2[df_traj2['tsid'] == i+1]['k']
        rdf_x = pandas2ri.py2rpy(df_traj_x)
        rdf_k = pandas2ri.py2rpy(df_traj_k)
        globalenv['x_time_series'] = rdf_x
        globalenv['k_time_series'] = rdf_k
        ...
        df_dev['k'] = dev_k
        # Export DEV as individual files for dynamical eigenvalue
        filepath_dev = '../data_us/food_hopf_white/{}/dev/food_hopf_500_dev_{}.csv'.format(kl,i+1)
        df_dev.to_csv(filepath_dev,index=False)
    ...
    # Create directories for output
    if not os.path.exists('../data_nus/food_hopf_white/{}/ews'.format(kl)):
        os.makedirs('../data_nus/food_hopf_white/{}/ews'.format(kl))

    if not os.path.exists('../data_nus/food_hopf_white/{}/resids'.format(kl)):
        os.makedirs('../data_nus/food_hopf_white/{}/resids'.format(kl))

    if not os.path.exists('../data_nus/food_hopf_white/{}/ac'.format(kl)):
        os.makedirs('../data_nus/food_hopf_white/{}/ac'.format(kl))

    if not os.path.exists('../data_us/food_hopf_white/{}/ews'.format(kl)):
        os.makedirs('../data_us/food_hopf_white/{}/ews'.format(kl))

    if not os.path.exists('../data_us/food_hopf_white/{}/resids'.format(kl)):
        os.makedirs('../data_us/food_hopf_white/{}/resids'.format(kl))

    if not os.path.exists('../data_us/food_hopf_white/{}/ac'.format(kl)):
        os.makedirs('../data_us/food_hopf_white/{}/ac'.format(kl))

    # Export EWS data
    df_ews_1.to_csv('../data_nus/food_hopf_white/{}/ews/df_ews_forced.csv'.format(kl))
    df_ews_2.to_csv('../data_us/food_hopf_white/{}/ews/df_ews_forced.csv'.format(kl))
    ...
    for i in np.arange(numSims)+1:
        df_resids = df_ews_1[(df_ews_1['tsid'] == i) & (df_ews_1['Variable'] == 'x')][['Time','Residuals','k']]
        df_ac = df_degf_1[(df_degf_1['tsid'] == i) & (df_degf_1['Variable'] == 'pca')][['Lag-1 AC','k']]
        df_ac = df_ac[df_ac['Lag-1 AC'].notna()]
        filepath_resids='../data_nus/food_hopf_white/{}/resids/food_hopf_500_resids_{}.csv'.format(kl,i)
        filepath_ac='../data_nus/food_hopf_white/{}/ac/food_hopf_500_ac_{}.csv'.format(kl,i)
        df_resids.to_csv(filepath_resids,index=False)
        df_ac.to_csv(filepath_ac,index=False)

    for i in np.arange(numSims)+1:
        df_resids = df_ews_2[(df_ews_2['tsid'] == i) & (df_ews_2['Variable'] == 'x')][['Time','Residuals','k']]
        df_ac = df_degf_2[(df_degf_2['tsid'] == i) & (df_degf_2['Variable'] == 'pca')][['Lag-1 AC','k']]
        df_ac = df_ac[df_ac['Lag-1 AC'].notna()]
        filepath_resids='../data_us/food_hopf_white/{}/resids/food_hopf_500_resids_{}.csv'.format(kl,i)
        filepath_ac='../data_us/food_hopf_white/{}/ac/food_hopf_500_ac_{}.csv'.format(kl,i)
        df_resids.to_csv(filepath_resids,index=False)
        df_ac.to_csv(filepath_ac,index=False)

    df_result_1.to_csv('../data_nus/food_hopf_white/{}/food_hopf_500_result.csv'.format(kl))
    df_result_2.to_csv('../data_us/food_hopf_white/{}/food_hopf_500_result.csv'.format(kl))

    print('kl = {} has finished'.format(kl))