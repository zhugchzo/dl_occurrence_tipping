...
for i in np.linspace(0,0.3,11):
   ...
   # Compute residual time series for irregularly-sampled time series
    ...
    for i in range(numSims):
        df_ews_temp = df_ews_1[(df_ews_1['tsid'] == i+1) & (df_ews_1['Variable'] == 'x')][['State variable','u']]
    ...
    for i in range(numSims):
        df_ews_temp = df_ews_2[(df_ews_2['tsid'] == i+1) & (df_ews_2['Variable'] == 'x')][['State variable','u']]
        x = df_ews_temp['State variable']
        u = df_ews_temp['u']
        ac_r,u_ac = ac_red(x,u,rw)
        df_ac = pd.DataFrame(data=None,columns=['AC red','u'])
