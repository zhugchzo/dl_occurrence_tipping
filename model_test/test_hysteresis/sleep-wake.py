...
ssf_vals = np.linspace(0.1,1.1,11)
ssr_vals = np.linspace(1.9,0.9,11)
...
for ssf,ssr in zip(ssf_vals,ssr_vals):
    ...
    f_resids = pandas.read_csv('../data_hysteresis/sleep-wake_white/sleep-wake_forward_{}.csv'.format(ssf))
    r_resids = pandas.read_csv('../data_hysteresis/sleep-wake_white/sleep-wake_reverse_{}.csv'.format(ssr))
    keep_col_resids = ['residuals_v','D']
    new_f_resids = f_resids[keep_col_resids]
    new_r_resids = r_resids[keep_col_resids]

    values_fs = new_f_resids['D'].iloc[0]
    values_fo = new_f_resids['D'].iloc[-1]

    values_rs = new_r_resids['D'].iloc[0]
    values_ro = new_r_resids['D'].iloc[-1]
